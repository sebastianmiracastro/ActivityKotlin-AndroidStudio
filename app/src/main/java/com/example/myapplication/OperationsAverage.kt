...
class OperationsAverage {
    ...
    fun registerStudents(SDS: StudentsData){
        MutableListStudent.add(SDS)
    }

    fun CountStudentsLists():Int{
        return MutableListStudent.count()
    }

    fun ValidateGradeStudent(Number:Double):Boolean{
        return Number in 0.0..5.0
    }
    ...
    fun statusStudent(SDS: StudentsData):String{
        var StudentStatus= ""
        if(SDS.averageSubGraStudent > 3.5){ StudentStatus = "El Estudiante Gano El Año" }
        else if(SDS.averageSubGraStudent > 2.5){ StudentStatus="El Estudiante Puede Recuperar" }
        else{ StudentStatus="El Estudiante Perdio, Pailas Hermano" }
